...
public class a extends JFrame implements ActionListener {
	...
	a() {
		jf = new JFrame(" Apna Java Editor -- OOM Project"); // JFrame Object is Formed
		jf.getContentPane().setBackground(new Color(47,79,79)); // Setting Its Background
		...
		jl.setBounds(1200, 170, 230, 25);
		jl.setFont(new Font("varinda", Font.PLAIN, 20));
		jl.setForeground(new Color(255, 250, 250));

		// Present Directory Label
		jl1 = new JLabel("Present Directory : ");
		jl1.setBounds(1200, 280, 200, 25);
		jl1.setFont(new Font("varinda", Font.PLAIN, 20));
		jl1.setForeground(new Color(255, 250, 250));

		// Output Label
		jl3 = new JLabel("Output : ");
		jl3.setBounds(1200, 435, 100, 25);
		jl3.setFont(new Font("varinda", Font.PLAIN, 20));
		jl3.setForeground(new Color(255, 250, 250));
		...
		// Java Class Name
		jtf = new JTextField();
		jtf.setBounds(1200, 200, 300, 25);
		jtf.setFont(new Font("varinda", Font.BOLD, 15));

		// Present Directory
		jtf1 = new JTextField();
		jtf1.setBounds(1200, 310, 300, 25);
		...
		jsp1.setBounds(1200, 475, 300, 150);
		...
		r = Runtime.getRuntime(); // Runtime class is a subclass of Object class, can provide access to various
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ((e.getSource() == jbcompile) || (e.getActionCommand().equals("Compile"))) {
			...
			if (!jtf.getText().equals("")) {
				...
				if (fileChooser.showSaveDialog(a.this) != JFileChooser.APPROVE_OPTION)
					return;
		...
		if ((e.getSource() == jbrun) || (e.getActionCommand().equals("Run"))) {
			...
			try {
				// Construct the run script content
				String runScript = "@echo off\n" +
						"java " + name2 + "\n" +
						"echo.\n" +
				...
				File runScriptFile = new File(filepath2 + "\\run_program.bat");
				try (PrintWriter writer = new PrintWriter(runScriptFile)) {
					writer.println(runScript);
				}

				// Execute the run script
				ProcessBuilder pbRun = new ProcessBuilder("cmd", "/C", "start", "cmd", "/k", "call",
						runScriptFile.getAbsolutePath());
				pbRun.directory(new File(filepath2));
		...
		if (e.getActionCommand().equals("About")) {
			...
			JLabel jl6;
			jl6 = new JLabel(
					"<html> Apna Java Editor - OOM <br><br> Grp Members : <br> Piyush Kedia <br> Aryan Shah <br> Sahil Sonawane</html>",
